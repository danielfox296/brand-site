...
**Title separator convention (all pages, ratified 2026-07-11):** `" | Entuned"`. The build normalizes this — for blog posts it strips any legacy `" — Entuned Blog"` / `" | Entuned"` suffix from the YAML/config title and appends `" | Entuned"`. Author titles WITHOUT a suffix. The old em-dash blog suffix is retired (it spent 15 chars of the ~60-char SERP budget on the word "Blog").
...
Blog posts are NOT section files: every post is a structured `content.yaml` (frontmatter + a `sections:` list of typed blocks — `prose`, `subhead`, `pullquote`, `stat_callout`, `cta`, etc.) rendered by `_src/lib/blog_renderer.py` through `_src/templates/blog_post.html`. See CLAUDE.md for the block types and canonical CTA. All 179 live posts use this format; the HTML-sections blog pattern is retired.
...
3. Add `content.yaml` in the structured YAML format (see CLAUDE.md — never `sections/01-content.html`)
...
7. Run `python3 build.py` — sitemap.xml, llms.txt, and rss.xml are all generated by the build (never hand-edit them; llms.txt's non-blog prose lives in `_src/llms-template.txt`)
...
The pricing CTA topology is intentionally asymmetric — see `CLAUDE.md` → "Pricing CTA topology" for the authoritative table and hierarchy:

- **Entuned Free** → `https://app.entuned.co/start` — **the primary CTA site-wide (2026-07-11): lead people to the listen-now path** (self-onboard activation, no card)
- **Boost / Pro** → direct Stripe Checkout (links live on the pricing page)
- **Multi-location pilot (5–50 doors)** → `pilot.html` → `contact.html?topic=pilot` — live secondary door on buyer-intent surfaces
- **Enterprise (>50 doors)** → `contact.html?topic=enterprise` (high-touch, inbound only)
